...
> **Drift guard:** run `node scripts/check-tokens.mjs` after editing any of these files. It verifies the JSON is valid, every color in `tokens.json` matches `DESIGN.md`, and every token a component binds in `registry.json` resolves to a real token. Since `DESIGN.md` is canonical, this is how the exports stay honest.
...
This rolls the ~120 component-token states in `DESIGN.md` up into ~50 logical components. It is the payload an agent loads *on demand* when it needs a specific component's shape — far cheaper than reading the full spec.
